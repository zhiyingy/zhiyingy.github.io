...
minimaxResult *alphabetaSearch(int curDepth, int maxDepth, int alpha, int beta,
    ...
    for (std::vector<move*>::iterator it = possibleMoves.begin(); it != possibleMoves.end(); it++) {
        ...
        minimaxResult *curRes = alphabetaSearch(curDepth + 1, maxDepth, -beta, -alpha, board,
...
minimaxResult *PVSplit(int curDepth, int maxDepth, int alpha, int beta,
    int **board, int curPlayer) {
    ...
    minimaxResult *firstRes = PVSplit(curDepth + 1, maxDepth, -beta, -alpha, board, flipPlayer(curPlayer));
    ...
    if (int(possibleMoves.size()) > 0)  {
        ...
        for (i = 0; i < possibleMoves.size(); i++) {
            ...
            if (flag) {
                ...
                minimaxResult *curRes = alphabetaSearch(curDepth + 1, maxDepth, -beta, -alpha, boardCopy, flipPlayer(curPlayer));
                int resS = -(curRes->bestRes);
...
move *calculateStepAB(int **board, int curPlayer) {
    ...
    minimaxResult *res;
    res = PVSplit(0, MAX_DEPTH, NEGINF, POSINF, board, curPlayer);
    move *m = res->mv;
    free(res);
    return m;
  // return NULL;
